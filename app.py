...
# ---> INCOLLA QUI IL TUO LINK GOOGLE SHEETS <---
...
# ---> INCOLLA QUI IL TUO LINK GOOGLE SHEETS <---
SHEET_URL = "INCOLLA_QUI_IL_TUO_LINK_CSV"

# 2. CARICAMENTO DATI DAL WEB
...
def load_data():
    try:
        ...
        st.error(f"Errore caricamento dati: Assicurati di aver pubblicato il CSV. Dettaglio: {e}")
        return pd.DataFrame()
...
if min_date and max_date:
    date_range = st.sidebar.date_input("Periodo Creazione:", [min_date, max_date])
else:
    date_range = []

regioni_disponibili = df['Region'].dropna().unique().tolist()
regioni_selezionate = st.sidebar.multiselect("Region/Paesi:", regioni_disponibili, default=regioni_disponibili)
...
if regioni_selezionate:
    df_filtered = df_filtered[df_filtered['Region'].isin(regioni_selezionate)]

# 5. MOTORE DI AGGREGAZIONE
